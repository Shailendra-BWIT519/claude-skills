...
description: Scaffold the solo-dev AI pipeline for a generic Node.js backend project (Express, Fastify, plain Node, etc — not NestJS, not a React/Next.js frontend) — pre-filled check.sh (typecheck/lint/test/format) and .claudeignore. Thin wrapper around the setup-ai-workflow skill's shared engine (run-plan.sh, classify-models.sh, hooks, commands) — same pipeline, Node-specific defaults so onboarding skips language detection. Use when the user wants this workflow set up in a Node.js backend project.
---
...
follow its Steps 1-8 exactly, with these Node-specific overrides to Step 5:

- **Skip language detection** — this is Node.js, confirmed by the user
...
- **`check.sh` starting commands** — fill in, then ALWAYS actually run
  ...
  - typecheck: `npx tsc --noEmit` if TypeScript (a `tsconfig.json` exists),
    ...
    `check.config`, delete the line in `check.sh`) — plain JS has nothing
    to typecheck.
...
- **a11y/visual-regression toggles are NOT relevant** — delete both
  `run_check` lines from `check.sh` entirely rather than leaving them off;
  this is a backend project with no rendered UI.
